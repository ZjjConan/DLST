function [ bb_samples ] = gen_samples(type, bb, n, imageSize, scaleFactor, trans_f, scale_f)
% GEN_SAMPLES
...
% 
    ...
    switch (type)
        case 'gaussian'
            ...
            samples(:,3:4) = samples(:,3:4) .* repmat(scaleFactor.^(scale_f*max(-1,min(1,0.5*randn(n,1)))),1,2);
        case 'uniform'
            samples(:,1:2) = samples(:,1:2) + trans_f * round(mean(bb(3:4))) * (rand(n,2)*2-1);
            samples(:,3:4) = samples(:,3:4) .* repmat(scaleFactor.^(scale_f*(rand(n,1)*2-1)),1,2);
        case 'uniform_aspect'
            samples(:,1:2) = samples(:,1:2) + trans_f * repmat(bb(3:4),n,1) .* (rand(n,2)*2-1);
            samples(:,3:4) = samples(:,3:4) .* scaleFactor.^(rand(n,2)*4-2);
            samples(:,3:4) = samples(:,3:4) .* repmat(scaleFactor.^(scale_f*rand(n,1)),1,2);
        case 'whole'
            ...
            windows = [dx(:) dy(:) bb(3)*scaleFactor.^ds(:) bb(4)*scaleFactor.^ds(:)];
